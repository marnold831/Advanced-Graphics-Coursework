...
Mesh::Mesh(GLuint type, bool instanced, size_t numInstances, std::vector<Matrix4> matrixes)
	: mappedBuffer(false), numVertices(0), type(type), texture(0), bumpTexture(0), 
	  vertices(nullptr), colours(nullptr), textureCoords(nullptr), normals(nullptr), 
	  tangents(nullptr), modelMatrixs(matrixes), indices(nullptr), numIndices(0), instanced(instanced),
...
Mesh* Mesh::GenerateShape(std::vector<Vector3>& vertices, std::vector<Vector4>& colours, GLuint type) {
	Mesh* m = new Mesh(type);
...
void Mesh::BufferData() {
	...
	if (instanced) {
		glGenBuffers(1, &bufferObject[MODEL_BUFFER]);
		glBindBuffer(GL_ARRAY_BUFFER, bufferObject[MODEL_BUFFER]);
		glBufferData(GL_ARRAY_BUFFER, sizeof(Matrix4) * numInstances, modelMatrixs.data(), GL_DYNAMIC_DRAW);

		for (size_t i = 0; i < 4; ++i) {
			glEnableVertexAttribArray(MODEL_BUFFER + i);
			glVertexAttribPointer(MODEL_BUFFER + i, 4, GL_FLOAT, GL_FALSE, sizeof(Matrix4), (const GLvoid*)(sizeof(GLfloat) * i * 4));
			glVertexAttribDivisor(MODEL_BUFFER + i, 1);
		}
	}
...
void Mesh::BufferDataMapped() {
	...
	if (bufferObject[COLOUR_BUFFER] == 0) {
		...
		if (ptr)
			memcpy(ptr, colours, numVertices * sizeof(Vector4));
